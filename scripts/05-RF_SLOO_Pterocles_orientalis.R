# modelling with Pterocles orientalis observations
...
foreach(i = 1:nrow(dataset[dataset$PA==1,]),
  ...
  # save training preds
  write.csv2(train_inbag['pred'], paste0('results/orientalis/train_preds',i,'.csv'), row.names = F)
  ...
  write.csv2(metrics_row, paste0("results/orientalis/metrics",i,".csv"), row.names=F)
  ...
  write.csv2(preds[,2], paste0("results/orientalis/preds",i,".csv"), row.names=F)
  # same for ssp126
  preds <- data.frame(pred = predict(model, newdata = env_df_ssp126, type ="prob"))
  write.csv2(preds[,2], paste0("results/orientalis/ssp126_preds",i,".csv"), row.names=F)
  # same for ssp585
  preds <- data.frame(pred = predict(model, newdata = env_df_ssp585, type ="prob"))
  write.csv2(preds[,2], paste0("results/orientalis/ssp585_preds",i,".csv"), row.names=F)
}
...
write.csv2(metrics, "results/metrics_ortega.csv", row.names=F)
...
write.csv2(preds,"results/pres_preds_ortega.csv", row.names=F)
# ssp126
preds <- data.frame(matrix(ncol = 0, nrow = nrow(env_df)))
...
write.csv2(preds,"results/ssp126_preds_ortega.csv", row.names=F)
# ssp585
preds <- data.frame(matrix(ncol = 0, nrow = nrow(env_df)))
...
write.csv2(preds,"results/ssp585_preds_ortega.csv", row.names=F)
...
foreach(i = 1:length(file_list),
        ...
        .packages=c("randomForest", "pdp")) %dopar% {
          ...
          write.csv2(pred, paste0("results/orientalis/pdp_prec",i,".csv"), row.names=F)
          # mean_temp
          pred <- partial(current_model, pred.var = "mean_temp", which.class = "1",
                          prob = T, train = dataset, plot = FALSE)
          write.csv2(pred, paste0("results/orientalis/pdp_mean_temp",i,".csv"), row.names=F)
          # mean_temp_seas
          pred <- partial(current_model, pred.var = "mean_temp_seas", which.class = "1",
                          prob = T, train = dataset, plot = FALSE)
          write.csv2(pred, paste0("results/orientalis/pdp_mean_temp_seas",i,".csv"), row.names=F)
          # prec_seas
          pred <- partial(current_model, pred.var = "prec_seas", which.class = "1",
                          prob = T, train = dataset, plot = FALSE)
          write.csv2(pred, paste0("results/orientalis/pdp_prec_seas",i,".csv"), row.names=F)
          # linear_infrastr
          pred <- partial(current_model, pred.var = "linear_infrastr", which.class = "1",
                          prob = T, train = dataset, plot = FALSE)
          write.csv2(pred, paste0("results/orientalis/pdp_linear_infrastr",i,".csv"), row.names=F)
          # slope
          pred <- partial(current_model, pred.var = "slope", which.class = "1",
                          prob = T, train = dataset, plot = FALSE)
          write.csv2(pred, paste0("results/orientalis/pdp_slope",i,".csv"), row.names=F)
          # forest (binary)
          pred <- partial(current_model, pred.var = "forest", which.class = c("0", "1"),
                          train = dataset, plot = FALSE)
          write.csv2(pred, paste0("results/orientalis/pdp_forest",i,".csv"), row.names=F)
          # int_perm_crop (binary)
          pred <- partial(current_model, pred.var = "int_perm_crop", which.class = c("0", "1"),
                          train = dataset, plot = FALSE)
          write.csv2(pred, paste0("results/orientalis/pdp_int_perm_crop",i,".csv"), row.names=F)
          # crop_med (binary)
          pred <- partial(current_model, pred.var = "crop_med", which.class = c("0", "1"),
                          train = dataset, plot = FALSE)
          write.csv2(pred, paste0("results/orientalis/pdp_crop_med",i,".csv"), row.names=F)
          # ext_perm_crop (binary)
          pred <- partial(current_model, pred.var = "ext_perm_crop", which.class = c("0", "1"),
                          train = dataset, plot = FALSE)
          write.csv2(pred, paste0("results/orientalis/pdp_ext_perm_crop",i,".csv"), row.names=F)
        }
toc() # 15094 sec
...
for (file in file_list) {
  int_perm_crop_file <- read.csv2(file)
