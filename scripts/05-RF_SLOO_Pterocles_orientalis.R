# modeling with Pterocles orientalis observations
...
env_df <- read.csv('data/envdf_present.csv')
env_df_ssp126 <- read.csv('data/envdf_ssp126.csv')
env_df_ssp585 <- read.csv('data/envdf_ssp585.csv')

# run models iterating through presence points only (324 models)
...
foreach(i = 1:nrow(dataset[dataset$PA==1,]),
        ...
        .packages=c('randomForest', 'sf', 'dplyr', 'pROC',
                    'caret', 'modEvA', 'stats')) %dopar% {
                      ...
                      # save the model
                      saveRDS(model, paste0('results/orientalis/model',i,'.rds'))
                      ...
                      # save training preds
                      write.csv(train_inbag['pred'], paste0('results/orientalis/train_preds',i,'.csv'), row.names = F)
                      ...
                      write.csv(metrics_row, paste0('results/orientalis/metrics',i,'.csv'), row.names=F)
                      ...
                      write.csv(preds[,2], paste0('results/orientalis/preds',i,'.csv'), row.names=F)
                      # same for ssp126
                      preds <- data.frame(pred = predict(model, newdata = env_df_ssp126, type ='prob'))
                      write.csv(preds[,2], paste0('results/orientalis/ssp126_preds',i,'.csv'), row.names=F)
                      # same for ssp585
                      preds <- data.frame(pred = predict(model, newdata = env_df_ssp585, type ='prob'))
                      write.csv(preds[,2], paste0('results/orientalis/ssp585_preds',i,'.csv'), row.names=F)
                    }
...
file_list <- list.files('results/orientalis', pattern = 'metrics', full.names = T)
for (file in file_list) {
  metrics_row <- t(read.csv(file))
  metrics <- rbind(metrics, metrics_row)
}
colnames(metrics) <- column_names
write.csv(metrics, 'results/metrics_ortega.csv', row.names=F)
# metrics <- read.csv('results/metrics_ortega.csv')

# join all preds columns into one dataframe
# present
preds <- data.frame(matrix(ncol = 0, nrow = nrow(env_df)))
file_list <- list.files('results/orientalis', pattern = 'preds', full.names = T)
file_list <- file_list[!grepl('126', file_list)]
file_list <- file_list[!grepl('585', file_list)]
for (file in file_list) {
  preds_col <- read.csv(file)
  preds <- cbind(preds, preds_col)
}
preds$mean <- rowMeans(preds)
preds_r <- rasterFromXYZ(cbind(env_df[,c('X','Y')], preds[,'mean']),
                         res=c(0.008333333, 0.008333333), crs=4326)
writeRaster(preds_r, 'results/ortega_preds.asc', format='ascii', overwrite=T)
# ssp126
preds <- data.frame(matrix(ncol = 0, nrow = nrow(env_df)))
file_list <- list.files('results/orientalis', pattern = 'ssp126_preds', full.names = T)
for (file in file_list) {
  preds_col <- read.csv(file)
  preds <- cbind(preds, preds_col)
}
...
preds_r <- rasterFromXYZ(cbind(env_df[,c('X','Y')], preds[,'mean']),
                         res=c(0.008333333, 0.008333333), crs=4326)
writeRaster(preds_r, 'results/ortega_ssp126_preds.asc', format='ascii', overwrite=T)
# ssp585
preds <- data.frame(matrix(ncol = 0, nrow = nrow(env_df)))
file_list <- list.files('results/orientalis', pattern = 'ssp585_preds', full.names = T)
for (file in file_list) {
  preds_col <- read.csv(file)
  preds <- cbind(preds, preds_col)
}
...
preds_r <- rasterFromXYZ(cbind(env_df[,c('X','Y')], preds[,'mean']),
                         res=c(0.008333333, 0.008333333), crs=4326)
writeRaster(preds_r, 'results/ortega_ssp585_preds.asc', format='ascii', overwrite=T)

# metrics for test predictions (only positive class)
# overall test sens
mean(metrics$bin_pred == 1) # 0.88
# overall cont pred
mean(metrics$cont_pred) # 0.76
# MAE
MAE(metrics$cont_pred, rep(1,nrow(metrics))) # 0.24
# MSE
mean((metrics$cont_pred - rep(1,nrow(metrics)))^2) # 0.12
# RMSE
RMSE(pred = metrics$cont_pred, obs = rep(1,nrow(metrics))) # 0.34
...
# accu <- read.csv('results/ortega_accu.csv')
...
ggsave('results/ortega_varimp.jpg', plot3, width = 10, height = 5, units = 'in')
...
foreach(i = 1:length(file_list),
        ...
        .packages=c('randomForest', 'pdp')) %dopar% {
          ...
          write.csv(pred, paste0('results/orientalis/pdp_prec',i,'.csv'), row.names=F)
          # mean_temp
          pred <- partial(current_model, pred.var = 'mean_temp', which.class = '1',
                          prob = T, train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/orientalis/pdp_mean_temp',i,'.csv'), row.names=F)
          # mean_temp_seas
          pred <- partial(current_model, pred.var = 'mean_temp_seas', which.class = '1',
                          prob = T, train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/orientalis/pdp_mean_temp_seas',i,'.csv'), row.names=F)
          # prec_seas
          pred <- partial(current_model, pred.var = 'prec_seas', which.class = '1',
                          prob = T, train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/orientalis/pdp_prec_seas',i,'.csv'), row.names=F)
          # linear_infrastr
          pred <- partial(current_model, pred.var = 'linear_infrastr', which.class = '1',
                          prob = T, train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/orientalis/pdp_linear_infrastr',i,'.csv'), row.names=F)
          # slope
          pred <- partial(current_model, pred.var = 'slope', which.class = '1',
                          prob = T, train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/orientalis/pdp_slope',i,'.csv'), row.names=F)
          # forest (binary)
          pred <- partial(current_model, pred.var = 'forest', which.class = c('0', '1'),
                          train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/orientalis/pdp_forest',i,'.csv'), row.names=F)
          # int_perm_crop (binary)
          pred <- partial(current_model, pred.var = 'int_perm_crop', which.class = c('0', '1'),
                          train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/orientalis/pdp_int_perm_crop',i,'.csv'), row.names=F)
          # crop_med (binary)
          pred <- partial(current_model, pred.var = 'crop_med', which.class = c('0', '1'),
                          train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/orientalis/pdp_crop_med',i,'.csv'), row.names=F)
          # ext_perm_crop (binary)
          pred <- partial(current_model, pred.var = 'ext_perm_crop', which.class = c('0', '1'),
                          train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/orientalis/pdp_ext_perm_crop',i,'.csv'), row.names=F)
        }
toc() # 15094 sec
...
file_list <- list.files('results/orientalis', pattern = 'prec', full.names = T)
file_list <- file_list[!grepl('seas', file_list)]
for (file in file_list) {
  prec_file <- read.csv(file)
...
# mean_temp
predictions_df <- data.frame(matrix(ncol = 2, nrow = 0))
file_list <- list.files('results/orientalis', pattern = 'mean_temp', full.names = T)
file_list <- file_list[!grepl('seas', file_list)]
for (file in file_list) {
  mean_temp_file <- read.csv(file)
...
# prec_seas
predictions_df <- data.frame(matrix(ncol = 2, nrow = 0))
file_list <- list.files('results/orientalis', pattern = 'prec_seas', full.names = T)
for (file in file_list) {
  prec_seas_file <- read.csv(file)
...
# mean_temp_seas
predictions_df <- data.frame(matrix(ncol = 2, nrow = 0))
file_list <- list.files('results/orientalis', pattern = 'mean_temp_seas', full.names = T)
for (file in file_list) {
  mean_temp_seas_file <- read.csv(file)
...
# slope
predictions_df <- data.frame(matrix(ncol = 2, nrow = 0))
file_list <- list.files('results/orientalis', pattern = 'slope', full.names = T)
for (file in file_list) {
  slope_file <- read.csv(file)
...
# linear_infrastr
predictions_df <- data.frame(matrix(ncol = 2, nrow = 0))
file_list <- list.files('results/orientalis', pattern = 'linear_infrastr', full.names = T)
for (file in file_list) {
  linear_infrastr_file <- read.csv(file)
...
# forest
predictions_df <- data.frame(matrix(ncol = 2, nrow = 0))
file_list <- list.files('results/orientalis', pattern = 'forest', full.names = T)
for (file in file_list) {
  forest_file <- read.csv(file)
...
# int_perm_crop
predictions_df <- data.frame(matrix(ncol = 2, nrow = 0))
file_list <- list.files('results/orientalis', pattern = 'int_perm_crop', full.names = T)
for (file in file_list) {
  int_perm_crop_file <- read.csv(file)
  predictions_df <- rbind(predictions_df, int_perm_crop_file)
}
# statistics for each level
summary_stats <- predictions_df %>%
  group_by(int_perm_crop) %>%
  summarise(median_yhat = median(yhat), q1_yhat = quantile(yhat, 0.25), q3_yhat = quantile(yhat, 0.75))
# boxplots for both levels
int_perm_crop <- ggplot(summary_stats, aes(x = factor(int_perm_crop), y = median_yhat)) +
  geom_point() +
  geom_errorbar(aes(ymin = q1_yhat, ymax = q3_yhat), width = 0.1) + # error bars
  theme_bw() +
  labs(x = 'int_perm_crop', y = 'Positive Prob')
print(int_perm_crop)

# crop_med
predictions_df <- data.frame(matrix(ncol = 2, nrow = 0))
file_list <- list.files('results/orientalis', pattern = 'crop_med', full.names = T)
for (file in file_list) {
  crop_med_file <- read.csv(file)
  predictions_df <- rbind(predictions_df, crop_med_file)
}
# statistics for each level
summary_stats <- predictions_df %>%
  group_by(crop_med) %>%
  summarise(median_yhat = median(yhat), q1_yhat = quantile(yhat, 0.25), q3_yhat = quantile(yhat, 0.75))
# boxplots for both levels
crop_med <- ggplot(summary_stats, aes(x = factor(crop_med), y = median_yhat)) +
  geom_point() +
  geom_errorbar(aes(ymin = q1_yhat, ymax = q3_yhat), width = 0.1) + # error bars
  theme_bw() +
  labs(x = 'crop_med', y = 'Positive Prob')
print(crop_med)

# ext_perm_crop
predictions_df <- data.frame(matrix(ncol = 2, nrow = 0))
file_list <- list.files('results/orientalis', pattern = 'ext_perm_crop', full.names = T)
for (file in file_list) {
  ext_perm_crop_file <- read.csv(file)
...
pdps <- grid.arrange(mean_temp, mean_temp_seas, prec, prec_seas, slope, linear_infrastr,
                     forest, int_perm_crop, crop_med, ext_perm_crop, nrow = 2, ncol = 5)
ggsave('results/pdps_ortega.jpg', pdps, width = 15, height = 5, units = 'in')
