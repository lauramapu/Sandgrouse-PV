...
write.csv(vifmodel, 'results/vif_iberica.csv', row.names=T)
# no vif values over 5

# generate dataframes for modeling

# present
dataset <- as.data.frame(raster::extract(env_stack, points))
...
# save
write.csv(dataset, 'data/iberica_PA_present.csv', row.names=F)
...
count(dataset[dataset$PA==1,])
# save
write.csv(dataset, 'data/ortega_PA_present.csv', row.names=F)
...
write.csv(env_df, 'data/envdf_present.csv', row.names=F)
...
# create two levels for PA
env_df$PA <- factor(env_df$PA, levels = c('0', '1'))
write.csv(env_df, 'data/envdf_ssp126.csv', row.names=F)
...
# create two levels for PA
env_df$PA <- factor(env_df$PA, levels = c('0', '1'))
write.csv(env_df, 'data/envdf_ssp585.csv', row.names=F)
