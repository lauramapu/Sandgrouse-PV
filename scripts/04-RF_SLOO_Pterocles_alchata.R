# modeling with Pterocles alchata observations
...
foreach(i = 1:nrow(dataset[dataset$PA==1,]),
        ...
        .packages=c('randomForest', 'sf', 'dplyr', 'pROC',
                    'caret', 'modEvA', 'stats')) %dopar% {
                      ...
                      # save training preds
                      write.csv(train_inbag['pred'], paste0('results/alchata/train_preds',i,'.csv'), row.names = F)
                      ...
                      # bin preds as factors because needed by confusion matrix
                      ...
                      # COR (point biserial correlation, dichotomous vs continuous)
                      ...
                      write.csv(metrics_row, paste0('results/alchata/metrics',i,'.csv'), row.names=F)
                      ...
                      write.csv(preds[,2], paste0('results/alchata/preds',i,'.csv'), row.names=F)
                      # same for ssp126
                      preds <- data.frame(pred = predict(model, newdata = env_df_ssp126, type ='prob'))
                      write.csv(preds[,2], paste0('results/alchata/ssp126_preds',i,'.csv'), row.names=F)
                      # same for ssp585
                      preds <- data.frame(pred = predict(model, newdata = env_df_ssp585, type ='prob'))
                      write.csv(preds[,2], paste0('results/alchata/ssp585_preds',i,'.csv'), row.names=F)
                    }
...
write.csv(metrics, 'results/metrics_iberica.csv', row.names=F)
# metrics <- read.csv('results/metrics_iberica.csv')

# join all preds into one ascii
# present
...
writeRaster(preds_r, 'results/iberica_preds.asc', format='ascii', overwrite=T)
# ssp126
preds <- data.frame(matrix(ncol = 0, nrow = nrow(env_df)))
...
preds_r <- rasterFromXYZ(cbind(env_df[,c('X','Y')], preds[,'mean']),
                         res=c(0.008333333, 0.008333333), crs=4326)
writeRaster(preds_r, 'results/iberica_ssp126_preds.asc', format='ascii', overwrite=T)
# ssp585
preds <- data.frame(matrix(ncol = 0, nrow = nrow(env_df)))
...
preds_r <- rasterFromXYZ(cbind(env_df[,c('X','Y')], preds[,'mean']),
                         res=c(0.008333333, 0.008333333), crs=4326)
writeRaster(preds_r, 'results/iberica_ssp585_preds.asc', format='ascii', overwrite=T)
...
# accu <- read.csv('results/iberica_accu.csv')
...
foreach(i = 1:length(file_list),
        ...
        .packages=c('randomForest', 'pdp')) %dopar% {
          ...
          write.csv(pred, paste0('results/alchata/pdp_prec',i,'.csv'), row.names=F)
          # mean_temp
          pred <- partial(model, pred.var = 'mean_temp', which.class = '1',
                          prob = T, train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/alchata/pdp_mean_temp',i,'.csv'), row.names=F)
          # mean_temp_seas
          pred <- partial(model, pred.var = 'mean_temp_seas', which.class = '1',
                          prob = T, train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/alchata/pdp_mean_temp_seas',i,'.csv'), row.names=F)
          # prec_seas
          pred <- partial(model, pred.var = 'prec_seas', which.class = '1',
                          prob = T, train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/alchata/pdp_prec_seas',i,'.csv'), row.names=F)
          # linear_infrastr
          pred <- partial(model, pred.var = 'linear_infrastr', which.class = '1',
                          prob = T, train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/alchata/pdp_linear_infrastr',i,'.csv'), row.names=F)
          # elev
          pred <- partial(model, pred.var = 'elev', which.class = '1',
                          prob = T, train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/alchata/pdp_elev',i,'.csv'), row.names=F)
          # slope
          pred <- partial(model, pred.var = 'slope', which.class = '1',
                          prob = T, train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/alchata/pdp_slope',i,'.csv'), row.names=F)
          # forest (binary)
          pred <- partial(model, pred.var = 'forest', which.class = c('0', '1'),
                          train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/alchata/pdp_forest',i,'.csv'), row.names=F)
          # crop_low (binary)
          pred <- partial(model, pred.var = 'crop_low', which.class = c('0', '1'),
                          train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/alchata/pdp_crop_low',i,'.csv'), row.names=F)
          # ext_perm_crop (binary)
          pred <- partial(model, pred.var = 'ext_perm_crop', which.class = c('0', '1'),
                          train = dataset, plot = FALSE)
          write.csv(pred, paste0('results/alchata/pdp_ext_perm_crop',i,'.csv'), row.names=F)
        }
toc() # 15094 sec
